...
install: settings install-kuadrant-manifest install-authorino-manifest install-example-config ## Install Kuadrant and Authorino manifest, and samples into the K8s cluster specified in ~/.kube/config.
...
settings: manifests kustomize ## Install ConfigMaps and Secrets required by Kamwiel
	GITHUB_ENDPOINT=$(GITHUB_ENDPOINT) GITHUB_TOKEN=$(GITHUB_TOKEN) $(KUSTOMIZE) build config/settings | kubectl -n $(CLUSTER_NAMESPACE) apply -f -

jobs: manifests kustomize ## Install CronJobs (i.e.: Webhook)
	$(KUSTOMIZE) build config/jobs | kubectl -n $(CLUSTER_NAMESPACE) apply -f -
